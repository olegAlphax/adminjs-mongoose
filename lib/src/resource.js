...
class Resource extends adminjs_1.BaseResource {
    ...
    async find(filters = {}, { limit = 20, offset = 0, sort = {} }, projection = {}) {
    ...
    async findMany(ids, projection = {}) {
        const mongooseObjects = await this.MongooseModel.find({ _id: ids }, { ...projection });
        return mongooseObjects.map(mongooseObject => (new adminjs_1.BaseRecord(Resource.stringifyId(mongooseObject), this)));
    }
    build(params) {
